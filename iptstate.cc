...
int main(int argc, char *argv[])
{
	...
	// Variables
	string line, src, dst, srcpt, dstpt, proto, code, type, state, ttl,
		mins, secs, hrs, sorting, tmpstring, format, prompt;
	 ...
	 * Initialize
	 */
	flags.single = flags.totals = flags.lookup = flags.skiplb
		= flags.staticsize = flags.skipdns = flags.tag_truncate
		= flags.filter_src = flags.filter_dst = flags.filter_srcpt
		= flags.filter_dstpt = flags.noscroll = flags.nocolor
		= flags.counters = false;
	ssize.x = ssize.y = 0;
	counts.tcp = counts.udp = counts.icmp = counts.other = counts.skipped
		= 0;
	...
	while (1) {
		 ...
		 * configured for
		 */
		print_table(stable, flags, format, sorting, filters, counts,
				ssize, max, mainwin, curr_state);
